document.addEventListener('DOMContentLoaded', function () {
  ...
  var pages = [
    ...
    { name: 'Why Learn Hindi?', href: 'why-learn-hindi.html' },
    { name: 'What I Offer',     href: 'what-i-offer.html' },
  ...
  function buildNavLinks() {
    return pages.map(function (p) {
      var isCurrent = p.name === currentPage;
      return '<li><a href="' + p.href + '"' +
        (isCurrent ? ' aria-current="page" class="active"' : '') +
        '>' + p.name + '</a></li>';
    }).join('');
  }
  ...
  document.querySelectorAll('[data-image-error]').forEach(function (img) {
    img.addEventListener('error', function () {
      var action = img.getAttribute('data-image-error');
      if (action === 'hide') {
        img.hidden = true;
      } else if (action === 'fallback' && img.nextElementSibling) {
        img.hidden = true;
  ...
  // Fade-up on scroll (with prefers-reduced-motion check)
  ...
  // Form validation
  var EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  var NZ_PHONE_RE = /^(\+64|0)(2\d|3|4|6|7|9)\d{6,9}$/;
  ...
  function handleFormSubmit(formId, fields, successId) {
    ...
    form.addEventListener('submit', function (e) {
      ...
      fields.forEach(function (f) {
        ...
        var msg = '';
        if (f.required && !val) { msg = f.emptyMsg; }
        else if (f.type === 'email' && val && !EMAIL_RE.test(val)) { msg = 'Please enter a valid email address, such as name@example.com.'; }
        else if (f.type === 'tel' && val && !NZ_PHONE_RE.test(val.replace(/\s/g, ''))) { msg = 'Please enter a valid New Zealand phone number, such as 021 123 4567 or +64 21 123 4567.'; }
        else if (f.minLen && val.length > 0 && val.length < f.minLen) { msg = f.minLenMsg; }
        else if (f.maxLen && val.length > f.maxLen) { msg = f.maxLenMsg; }
        if (msg) { showError(input, errorEl, msg); valid = false; if (!firstInvalid) firstInvalid = input; }
        else { clearError(input, errorEl); }
        input.addEventListener('input', function () { if (input.value.trim()) clearError(input, errorEl); });
      });

      if (formId === 'contact-form') {
        var privacyCheck = document.getElementById('contact-privacy');
        var privacyErr = document.getElementById('contact-privacy-error');
        if (privacyCheck && privacyErr) {
          if (!privacyCheck.checked) { privacyErr.textContent = 'Please confirm that you understand how your information will be used.'; privacyErr.hidden = false; valid = false; if (!firstInvalid) firstInvalid = privacyCheck; }
          else { privacyErr.hidden = true; }
        }
      }

      if (formId === 'booking-form') {
        var levelEl = document.getElementById('booking-level');
        var levelErr = document.getElementById('booking-level-error');
        if (levelEl && levelErr) {
          if (!levelEl.value) { levelErr.textContent = 'Please select your current Hindi level so Anuradha can tailor your lessons.'; levelErr.hidden = false; valid = false; if (!firstInvalid) firstInvalid = levelEl; }
          else { levelErr.hidden = true; }
        }
        var reasonChecks = form.querySelectorAll('input[name="reason"]');
        var reasonErr = document.getElementById('booking-reason-error');
        var anyChecked = false;
        reasonChecks.forEach(function (cb) { if (cb.checked) anyChecked = true; });
        if (reasonErr) {
          if (!anyChecked) { reasonErr.textContent = 'Please select at least one reason for learning Hindi.'; reasonErr.hidden = false; valid = false; }
          else { reasonErr.hidden = true; }
        }
        var bookPrivacy = document.getElementById('booking-privacy');
        var bookPrivacyErr = document.getElementById('booking-privacy-error');
        if (bookPrivacy && bookPrivacyErr) {
          if (!bookPrivacy.checked) { bookPrivacyErr.textContent = 'Please confirm that you understand how your information will be used.'; bookPrivacyErr.hidden = false; valid = false; if (!firstInvalid) firstInvalid = bookPrivacy; }
          else { bookPrivacyErr.hidden = true; }
        }
        var msgEl = document.getElementById('booking-message');
        var msgErr = document.getElementById('booking-message-error');
        if (msgEl && msgErr && msgEl.value.length > 1000) { msgErr.textContent = 'Your message must be no longer than 1000 characters.'; msgErr.hidden = false; valid = false; if (!firstInvalid) firstInvalid = msgEl; }
      }
  ...
  handleFormSubmit('contact-form', [
    { id: 'contact-name',  errorId: 'contact-name-error',  required: true,  type: 'text',  emptyMsg: 'Please enter your name.',         minLen: 2, minLenMsg: 'Your name must be at least 2 characters.', maxLen: 100, maxLenMsg: 'Your name must be no longer than 100 characters.' },
    { id: 'contact-email', errorId: 'contact-email-error', required: true,  type: 'email', emptyMsg: 'Please enter your email address.' },
    { id: 'contact-phone', errorId: 'contact-phone-error', required: false, type: 'tel' }
  ], 'contact-success');

  handleFormSubmit('booking-form', [
    { id: 'booking-name',  errorId: 'booking-name-error',  required: true,  type: 'text',  emptyMsg: 'Please enter your full name.',     minLen: 2, minLenMsg: 'Your name must be at least 2 characters.', maxLen: 100, maxLenMsg: 'Your name must be no longer than 100 characters.' },
    { id: 'booking-email', errorId: 'booking-email-error', required: true,  type: 'email', emptyMsg: 'Please enter your email address.' },
    { id: 'booking-phone', errorId: 'booking-phone-error', required: false, type: 'tel' }
  ], 'booking-success');
